...
export function parseFeishuMessageEvent(
  ...
  const ctx: FeishuMessageContext = {
    ...
    senderId: event.sender.sender_id.union_id || senderUserId || senderOpenId || "",
...
export async function handleFeishuMessage(params: {
  ...
  // before the async persistent check completes (per-account to avoid cross-app interference).
  const messageId = event.message.message_id;
  const memoryDedupeKey = `${account.accountId}:${messageId}`;
  if (!tryRecordMessage(memoryDedupeKey)) {
    log(
      `feishu: skipping duplicate message ${messageId} (memory dedup, account=${account.accountId})`,
    );
    return;
  }
  ...
  } else if (feishuCfg?.resolveSenderNames ?? true) {
  ...
  try {
    ...
    if (broadcastAgents) {
      ...
      const { dispatcher, replyOptions, markDispatchIdle } = createFeishuReplyDispatcher({
        ...
        runtime: runtime as RuntimeEnv,
        chatId: ctx.chatId,
        replyToMessageId: params._crossBotDepth ? undefined : replyTargetMessageId,
        skipReplyToInMessages: !isGroup,
        replyInThread,
        rootId: ctx.rootId,
        threadReply,
